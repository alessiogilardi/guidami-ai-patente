...
| **quiz item** | One flattened true/false statement to answer, after `FlattenQuiz` runs on a raw parsed quiz question (which may bundle multiple statements under one image). |
