...
| `guidami_ai_patente_ingestor/` — quiz indexing flow (`build_quiz_indexing_flow` with `ApplyStep(DeduplicateQuizItems(), ForEach(QuizMapper.from_enriched_to_embeddable))` + `ApplyStep(ForEach(...))`, `DbStoreStep` truncate full-reload; `MapToEmbeddableStep` REMOVED → `ToEmbeddableQuiz` service UseCase (SP03) → `ToEmbeddableQuiz` itself REMOVED, dedup promoted to the shared `DeduplicateQuizItems` service in `services/quiz/`) | ✅ implemented |
...
| `guidami_ai_patente_ingestor/` — quiz mapper consolidation (`QuizMapper` single; `from_enriched_to_embeddable(item)` 1 argument, flat model; unnest+map in `FlattenQuiz` service, dedup in shared `DeduplicateQuizItems` service, no longer in steps) | ✅ implemented |
...
| `guidami_ai_patente_ingestor/` — two-stage quiz preparation flow (SP09: `build_quiz_cleaning_flow` with `ApplyStep(FlattenQuiz(), DeduplicateQuizItems())` `parsed`→`cleaned`; SP04: `FlattenQuizStep` removed, logic moved to `FlattenQuiz` service UseCase) | ✅ implemented |
| `guidami_ai_patente_ingestor/` — SP04 refactor: `ApplyStep+ForEach` replace `MapStep`/`EnrichDataStep` in all flows; `FlattenQuiz`/`ToEmbeddableQuiz` service UseCase (SP02/SP03, `ToEmbeddableQuiz` later removed, dedup promoted to the shared `DeduplicateQuizItems` service — see quiz indexing flow row above); enrichers implement `UseCase` (SP01); `EnricherProtocol` removed | ✅ implemented |
