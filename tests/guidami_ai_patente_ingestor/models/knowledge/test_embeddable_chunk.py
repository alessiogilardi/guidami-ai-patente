...
def _chunk(**kwargs) -> EmbeddableChunkModel:
    defaults = dict(
        source="cds",
        article_number="1",
...
def test_embeddable_chunk_defaults_embedding_to_none() -> None:
    chunk = _chunk()
    assert chunk.embedding is None


def test_embeddable_chunk_defaults_context_to_empty_string() -> None:
    chunk = _chunk()
