...
def _sub_question(
    number: str, image: str | None = None, image_description: str | None = None
) -> EnrichedQuizItemModel:
...
def _main_question(
    question_id: int, sub_questions: list[EnrichedQuizItemModel]
) -> EnrichedQuizModel:
    return EnrichedQuizModel(
        question_id=question_id, topic="Segnaletica", sub_questions=sub_questions
    )
...
def test_enrich_dedups_calls_for_repeated_image(tmp_path: Path) -> None:
    ...
    questions = [
        _main_question(
            1,
            [
                _sub_question("1", image="a.jpeg"),
                _sub_question("2", image="a.jpeg"),
                _sub_question("3", image="b.jpeg"),
            ],
        )
    ]
...
def test_enrich_sets_formatted_description_on_matching_sub_questions(tmp_path: Path) -> None:
    ...
    enricher = ImageDescriptionEnricher(describer, tmp_path)
    questions = [_main_question(1, [_sub_question("1", image="stop.jpeg")])]

    result = enricher.enrich(questions)

    assert result[0].sub_questions[0].image_description == "Stop. Segnale rosso ottagonale."


def test_enrich_missing_file_skips_and_warns(tmp_path: Path, caplog) -> None:
    describer = _make_describer()
    enricher = ImageDescriptionEnricher(describer, tmp_path)
    questions = [_main_question(1, [_sub_question("1", image="missing.jpeg")])]
...
def test_enrich_no_image_means_description_stays_none(tmp_path: Path) -> None:
    describer = _make_describer()
    enricher = ImageDescriptionEnricher(describer, tmp_path)
    questions = [_main_question(1, [_sub_question("1", image=None)])]
...
def test_enrich_does_not_mutate_input_models(tmp_path: Path) -> None:
    ...
    describer = _make_describer()
    enricher = ImageDescriptionEnricher(describer, tmp_path)
    original = _main_question(1, [_sub_question("1", image="stop.jpeg")])
    questions = [original]

    enricher.enrich(questions)

    assert original.sub_questions[0].image_description is None
