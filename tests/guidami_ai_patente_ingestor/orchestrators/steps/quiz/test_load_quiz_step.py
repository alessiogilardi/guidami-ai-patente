...
from guidami_ai_patente_ingestor.orchestrators.steps.quiz import LoadQuizStep
...
def _make_main_question(question_id: int) -> QuizBankModel:
    return MagicMock(spec=QuizBankModel, question_id=question_id)


def test_required_keys_is_empty_set() -> None:
    repo = MagicMock(spec=QuizBankRepository)
    resolver = _make_layer_resolver()
    step = LoadQuizStep("load_quiz", repo, resolver, input_layer="cleaned", source="quiz")
...
def test_execute_loads_source_and_puts_list_in_context() -> None:
    ...
    result: list[QuizBankModel] = context.get(context_keys.CLEANED_QUIZ)
...
def test_execute_resolves_path_for_the_configured_source() -> None:
    ...
    resolver.path.assert_called_once_with("cleaned", "quiz")
...
def test_execute_passes_resolved_path_to_repository() -> None:
    ...
    repo = MagicMock(spec=QuizBankRepository)
    repo.load.return_value = []
    expected_path = Path("/fake/cleaned/quiz/quiz.json")
    resolver = MagicMock(spec=LayerResolver)
