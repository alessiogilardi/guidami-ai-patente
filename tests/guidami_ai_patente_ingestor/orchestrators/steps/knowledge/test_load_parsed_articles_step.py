"""Test per LoadParsedArticlesStep (per-source)."""
...
from guidami_ai_patente_ingestor.orchestrators.steps.knowledge import LoadParsedArticlesStep
...
def test_required_keys_is_empty_set() -> None:
    ...
    step = LoadParsedArticlesStep("load", repo, resolver, input_layer="parsed", source="cds")
    assert step.get_required_keys() == set()


def test_produced_keys_contains_parsed_articles() -> None:
    repo = MagicMock(spec=ArticleRepository)
    resolver = _make_layer_resolver()
    step = LoadParsedArticlesStep("load", repo, resolver, input_layer="parsed", source="cds")
    assert step.get_produced_keys() == {context_keys.PARSED_ARTICLES}
...
def test_execute_loads_source_and_puts_flat_list_in_context() -> None:
    ...
    result: list[Article] = context.get(context_keys.PARSED_ARTICLES)
...
def test_execute_resolves_path_for_the_configured_source() -> None:
    ...
    step = LoadParsedArticlesStep("load", repo, resolver, input_layer="parsed", source="cap")
    step.execute(FlowContext())

    resolver.path.assert_called_once_with("parsed", "cap")
