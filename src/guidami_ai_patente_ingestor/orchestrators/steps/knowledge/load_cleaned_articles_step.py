"""Step che carica gli articoli cleaned da disco per una singola source."""
...
class LoadCleanedArticlesStep(Step):
    """Carica gli articoli cleaned da disco per UNA source e li espone nel context.

    La preparation è per-source (una run per source): produce `CLEANED_ARTICLES`,
    una lista piatta di `Article`. Non richiede chiavi in input: è il primo
    step del flow di enrichment.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            article_repository: Repository per la lettura degli articoli cleaned da JSON.
            layer_resolver: Resolver che mappa (layer, source) → Path del file JSON.
            input_layer: Nome del layer di input (costante "cleaned").
    ...
    def execute(self, context: FlowContext) -> None:
        """Carica gli articoli della source e scrive la lista in `CLEANED_ARTICLES`.
        ...
        logger.info(f"Loaded {len(articles)} cleaned articles for source '{self._source}'")

        context.put(context_keys.CLEANED_ARTICLES, articles)
    ...
    def get_produced_keys(self) -> set[str]:
        """Produce `CLEANED_ARTICLES`: lista piatta degli articoli della source."""
